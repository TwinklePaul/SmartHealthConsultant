...
class SignUp_Pharmacy : Base_Activity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_signup_pharmacy)
    ...
    fun Save_record(view: View) {
        ...
        if (id.trim() != " " && name.trim() != " " && location.trim() != " " && pass.trim() != " ") {
            val status =
                databaseHandler.addPharmacy(Pharmacy_Model(id, name, location, cont, pass))
